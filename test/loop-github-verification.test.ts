...
test("GitHub PR checks verifier records checker verification from gh JSON", async () => {
  ...
  try {
    ...
    assert.equal(verification.evidence?.system, "github");
    ...
    assert.equal(metrics.verification.by_provider.some((group) => group.key === "checker" && group.verification.pass === 1), true);
    assert.equal(metrics.by_system.some((group) => group.key === "github" && group.verification.pass === 1), true);
  } finally {
